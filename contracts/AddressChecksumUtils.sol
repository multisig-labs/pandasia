...
library AddressChecksumUtils {
   ...
   * "0x" is not included.
   */
  function getChecksum(address account) external pure returns (string memory accountChecksum) {
   ...
   * checksum.
   */
  function getChecksumCapitalizedCharacters(address account) external pure returns (bool[40] memory characterCapitalized) {
   ...
   */
  function isChecksumValid(string calldata accountChecksum) external pure returns (bool ok) {
